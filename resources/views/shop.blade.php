...
<section class="bg-gray-50 py-8 antialiased md:py-12">
    <div class="mx-auto max-w-screen-xl px-4 2xl:px-0">
        ...
        <form class="max-w-md mx-auto">
            ...
            <div class="relative">
                ...
                <input autocomplete="off" name="search" type="search" id="default-search" class="block w-full p-4 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-primary-500 focus:border-primary-500" placeholder="Search Product" required />
                <button type="submit" class="text-white absolute end-2.5 bottom-2.5 bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2">Search</button>
            </div>
        </form>
        <!-- Heading & Filters -->
                ...
                <div class="pt-6">
                    <div class="mb-4 flex items-center justify-between gap-4">
                        <a href="/shop?category={{$product->category->name}}">
                            <span class="me-2 rounded bg-primary-100 px-2.5 py-0.5 text-xs font-medium text-primary-800">{{$product->category->name}}</span>
                        </a>
                    ...
                    <a href="{{ route('detail', $product->id) }}" class="text-lg font-semibold leading-tight text-gray-900 hover:underline">{{ $product->name }}</a>
                    ...
                    <div class="mt-4 flex items-center justify-between gap-4">
                        <p class="text-xl font-bold leading-tight text-gray-900">Rp. {{ number_format($product->price, 0, ',', '.') }}</p>

                        <input type="number" id="quantity" name="quantity" min="1" value="1" class="border rounded-lg w-16 text-center me-4 hidden" />
                        <button
    ...
    <!-- Filter modal -->
    <form action="{{ route('shop.index') }}" method="GET" id="filterModal" tabindex="-1" aria-hidden="true" class="fixed left-0 right-0 top-0 z-50 hidden h-modal w-full overflow-y-auto overflow-x-hidden p-4 md:inset-0 md:h-full">
        <div class="relative h-full w-full max-w-xl md:h-auto">
            ...
            <div class="relative rounded-lg bg-white shadow">
                ...
                <div class="px-4 md:px-5">
                    ...
                    <div id="myTabContent">
                        <div class="grid grid-cols-2 gap-4 md:grid-cols-3" id="Category" role="tabpanel" aria-labelledby="Category-tab">
                            ...
                            <div class="space-y-2">
                                <div class="flex items-center">
                                    <input id="category-{{ $category->id }}" name="category[]" type="checkbox" value="{{ $category->id }}" class="h-4 w-4 rounded border-gray-300 bg-gray-100 text-primary-600 focus:ring-2 focus:ring-primary-500" />
                                    <label for="category-{{ $category->id }}" class="ml-2 text-sm font-medium text-gray-900">{{ $category->name }} ({{ $category->products->count() }})</label>
                                </div>
                            </div>
                            @endforeach
                        </div>
                ...
                <div class="flex items-center space-x-4 rounded-b p-4 md:p-5">
                    <button type="submit" class="rounded-lg bg-primary-700 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-primary-800 focus:outline-none focus:ring-4 focus:ring-primary-300">Show results</button>
